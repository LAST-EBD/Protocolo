import os, shutil, re, time, subprocess, pandas, rasterio
import numpy as np
...
class Protocolo(object):
    ...
    def nor1(self, banda, mascara, std = 11):
        ...
        #parametros = {}
        #lista_nor1bis = [nor1(banda, 22), nor1bis(banda, 11), nor1bis(banda, 22), nor1(banda, 33), nor1bis(banda, 33)]

        #mascaras
        MASK_1 = mascara #Areas Pseudo Invariantes
        mask_nubes = r'C:\Users\Diego\Desktop\Landsat_8\20051217l7eTM202_34_CM.img'
        if MASK_1 is self.equilibrado:
        ...
        if banda_num in dnorbandas.keys():
            with rasterio.open(banda) as src:
                current = src.read()
                print banda, current.mean(), current.min(), current.max()
            ...
            if r_value > 0.85 and min(values.values()) >= 10:
                ...
                self.nor2(banda, slope, intercept)
    ...
    def nor2(self, banda, slope, intercept):
        ...
        banda_num = banda[-6:-4]
        outFile = os.path.join(self.nor, self.escena + '_grn1_' + banda_num + '.img')
